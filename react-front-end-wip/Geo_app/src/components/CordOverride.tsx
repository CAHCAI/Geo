...
const OverridesPage: React.FC = () => {
  ...
  return (
    <div className="min-h-screen flex flex-col items-center p-6 bg-white text-gray-900 dark:bg-[#1E1F22] dark:text-gray-200">
      <h1 className="text-3xl font-semibold text-gray-800 dark:text-white">Manual Overrides</h1>

      {/* Excel File Upload Section */}
      <section className="bg-gray-50 dark:bg-[#2B2D31] rounded-lg shadow-lg p-6 mt-6 w-full max-w-3xl mx-auto">
        <h2 className="text-xl font-semibold text-gray-700 dark:text-white mb-4">Excel File Upload</h2>
        <div className="bg-gray-50 dark:bg-[#1E1F22] flex flex-col items-center justify-center space-y-4">
          <div
            className={`border-2 border-dashed ${
              isDragging ? "border-blue-500" : "border-gray-300 dark:border-[#3C3F45]"
            } rounded-lg p-8 w-full text-center bg-gray-100 dark:bg-[#2B2D31] hover:bg-gray-200 dark:hover:bg-[#3A3C41] transition`}
          ...
          >
            <p className="text-gray-600 dark:text-gray-400 mb-4">Drag and drop your .xlsx file here</p>
            <input type="file" id="file-upload" className="hidden" accept=".xlsx" onChange={handleFileChange} />
            <label
              htmlFor="file-upload"
      ...
      <section className="bg-gray-50 dark:bg-[#2B2D31] rounded-lg shadow-lg p-6 mb-6 mt-10 w-full max-w-3xl mx-auto">
        <h2 className="text-xl font-semibold text-gray-700 dark:text-white mb-4">Coordinate Override</h2>
        <input
          type="text"
          placeholder="Enter coordinates (lat, lon)"
          value={coordinates}
          onChange={handleCoordinatesChange}
          className="w-full p-3 border border-gray-300 dark:border-[#3C3F45] rounded-lg mb-4 bg-white dark:bg-[#1E1F22] text-black dark:text-white placeholder-gray-400"
        />
        {showAddressInput && (
          <input
            type="text"
      ...
      {/* Overrides List */}
      <div className="bg-gray-50 dark:bg-[#2B2D31] rounded-lg shadow-lg p-6 mt-6 w-full max-w-3xl mx-auto max-h-[40vh] overflow-y-auto">
        {isLoading ? (
          ...
          overrides.map((override) => (
            <motion.div
              key={override.id}
              className="bg-gray-50 dark:bg-[#1E1F22] shadow-md p-4 rounded-lg flex justify-between items-center hover:dark:bg-[#2A2C30]"
              whileHover={{ scale: 1.02 }}
            >
              <div className="flex-1 mr-4">
                <p className="font-semibold truncate pr-2 max-w-[300px] text-gray-800 dark:text-white">
                  {override.address}
                </p>
                <p className="text-sm text-gray-500 dark:text-gray-400">
      ...
      <Dialog open={deleteItemId !== null} onClose={() => setDeleteItemId(null)}>
        <div className="fixed inset-0 flex items-center justify-center bg-gray-900 bg-opacity-50 z-50">
          <div className="bg-white dark:bg-[#2B2D31] text-gray-900 dark:text-white p-6 rounded-lg shadow-lg">
            ...
            <Dialog.Description className="text-sm text-gray-600 dark:text-gray-400 mt-2">
              Are you sure you want to delete this override entry? (You must click "Delete" below to confirm.)
            </Dialog.Description>
      ...
      <Dialog open={showModal} onClose={() => setShowModal(false)}>
        <div className="fixed inset-0 flex items-center justify-center bg-gray-900 bg-opacity-50 z-50">
          <div className="bg-white dark:bg-[#2B2D31] text-gray-900 dark:text-white p-6 rounded-lg shadow-lg w-96 relative">
            <button
            ...
            <div className="mt-2">
              <input
                type="text"
                className="border border-gray-300 dark:border-[#3C3F45] bg-white dark:bg-[#1E1F22] text-black dark:text-white rounded w-full p-2 mt-2"
                placeholder="Address"
      ...
      {showConfirmation && (
        ...
        >
          <div className="bg-white dark:bg-[#2B2D31] text-gray-900 dark:text-white p-6 rounded-lg shadow-lg">
            ...
            <button
              onClick={handleCoordSubmit}
              className="px-6 py-3 bg-green-500 hover:bg-green-600 text-white font-bold rounded-lg shadow-md transition mr-4"
              disabled={isSubmitting}
            >
              {isSubmitting ? (
